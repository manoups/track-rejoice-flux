    ...
    class ContentQueryTests {
        ...
        void searchPaginatedContent() {
            ...
            testFixture.givenCommands(contents)
                    ...
                    .mapResult(stats -> stats.get("details/subtype"))
                    .expectResult(facetStats -> facetStats.size() == 2 &&
                            facetStats.stream().filter(it -> it.value().equalsIgnoreCase("dog")).findFirst().orElseThrow(() -> new AssertionError("No pet facet found")).count() == 15 &&
                            facetStats.stream().filter(it -> it.value().equalsIgnoreCase("keys")).findFirst().orElseThrow(() -> new AssertionError("No keys facet found")).count() == 10);
        }
    }
