...
import com.breece.coreapi.user.api.query.IsDefaultAdmin;
...
//                    .firstName(decodedJWT.getClaim("given_name").asString())
                    .name(decodedJWT.getClaim("family_name").asString())
                    .email(email).build(), admin ? Role.ADMIN : Role.OWNER));
        }
...
//                .withClaim("given_name", userProfile.getDetails().getFirstName())
                .withClaim("family_name", userProfile.details().name())
                .withClaim("email", userProfile.details().email());
