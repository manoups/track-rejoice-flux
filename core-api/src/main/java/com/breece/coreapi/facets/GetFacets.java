...
public record GetFacets(@JsonTypeInfo(use = JsonTypeInfo.Id.CLASS) FacetedSearch<?, ?> query)
        implements FacetableRequest, Request<GetFacetStatsResult> {

    @HandleQuery
    @LocalHandler(logMetrics = true)
    GetFacetStatsResult getFacets(Sender sender) {
        return ofNullable(getSearch(sender)).map(Search::facetStats).map(GetFacetStatsResult::new).orElse(null);
    }
