...
public record GetFacets(
        @JsonTypeInfo(use = JsonTypeInfo.Id.CLASS) FacetedSearch<?, ?> query) implements FacetableRequest, Request<GetFacetStatsResult> {
    ...
    GetFacetStatsResult getFacets(Sender sender) {
        return ofNullable(getSearch(sender)).map(Search::facetStats).map(this::mapToFacetResult).orElse(null);
    }
    ...
    private GetFacetStatsResult mapToFacetResult(List<FacetStats> stats) {
        return new GetFacetStatsResult(nextRequestId.getAndIncrement(), stats);
    }
}
