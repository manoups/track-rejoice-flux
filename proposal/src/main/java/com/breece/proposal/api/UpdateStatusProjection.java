...
public record UpdateStatusProjection(LinkedSightingId linkedSightingId, @NotNull LinkedSightingStatus status) implements LinkedSightingSeekerUpdate {
    @Apply
    LinkedSighting update(LinkedSighting linkedSighting) {
        return linkedSighting.withStatus(status);
    }
}
